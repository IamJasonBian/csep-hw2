...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        for index, (x_i, y_i) in enumerate(zip(X, y)):
            mu_i = 1 / (1 + np.exp(-y_i * (self.bias + x_i.T.dot(self.weight))))
            mu.append(mu_i)
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for index, (x_i, y_i) in enumerate(zip(X, y)):
            loss_i = np.log(1 + np.exp(-y_i * (self.bias + x_i.T.dot(self.weight)))) + self._lambda*np.dot(self.weight, self.weight)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        for index, (mu_i, y_i, x_i) in enumerate(zip(mu, y, X)):
            mu_ls.append(mu_i * (1-(1/mu_i)) * y_i * x_i.T + 2 * self._lambda * np.absolute(self.weight))
        return sum(mu_ls)/len(mu_ls)
    ...
    def gradient_J_bias(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for index, (mu_i, y_i, x_i) in enumerate(zip(mu, y, X)):
            mu_ls.append(mu_i * (1-(1/mu_i)) * y_i)
        return sum(mu_ls)/len(mu_ls)
